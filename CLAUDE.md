...
- **`VolverAlInicio`**: la estrella del logo se transforma en un triángulo apuntando a la izquierda. Es un morph real de la propiedad `d`, y solo funciona porque **ambos trazados comparten estructura** — cuatro segmentos cúbicos con los mismos anclajes. Si se edita uno hay que mantener esa correspondencia o el icono deja de interpolar y salta. La etiqueta va siempre visible: el icono solo no comunica a dónde lleva
